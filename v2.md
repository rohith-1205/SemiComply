...
## 14. Version 2 Extended Features

Version 2 includes the production-oriented extensions implemented after the original lifecycle server specification.

### 14.1 Provider-agnostic email delivery

The notification module separates email delivery from incident business logic:

- `EmailProvider` defines `send()` and `verify()`.
- `SmtpProvider` is the current implementation using `nodemailer`.
- Gmail works through Gmail SMTP with an App Password.
- Outlook or another provider can be added by implementing `EmailProvider`; stakeholder, severity, audit, and email-generation logic do not need to change.
- `EmailService` delegates to the configured provider and supports provider replacement.

### 14.2 Stakeholder resolution and safe recipient selection

Stakeholders are loaded from the published `Stakeholders` tab using:

```text
name,email,role,notifyOn
```

Role-targeted requests use `recipientRoles`. Matching is case-insensitive and tolerates punctuation, whitespace, hidden Unicode characters, and name/role differences. For example, `Design Lead` can match a row whose `name` is `Design Lead` and whose `role` is `Design`.

The system never invents an email address. If no matching stakeholder is found, the tool returns an error and sends nothing. `customRecipients` is used only when the caller supplies the actual name, email, and role.

### 14.3 Notification modes

| Mode | Input | Behavior |
|---|---|---|
| Severity-based | `lotId`, optional `shipmentId` | Sends to stakeholders whose `notifyOn` includes the calculated severity |
| All stakeholders | `allStakeholders: true` | Sends to every valid stakeholder, ignoring severity filters |
| Role-targeted | `recipientRoles: ["Design Lead"]` | Sends only to matching stakeholder names/roles, independent of severity |

Use `dryRun: true` with any mode to preview recipients without sending. Every real delivery creates a row in `Notification Audit` with delivery status and provider message ID.

### 14.4 Robust Google Sheets handling

The CSV reader now handles Windows CRLF line endings and preserves extra comma-separated values in the final declared column, allowing unquoted `notifyOn` entries such as `HIGH,CRITICAL`. Quoted and uppercase severity values are normalized before filtering.

Google Sheets API writes accept service-account credentials from `GOOGLE_CLIENT_EMAIL` and `GOOGLE_PRIVATE_KEY` in `.env`, with `GOOGLE_SERVICE_ACCOUNT_PATH` retained as a file-based fallback. Literal `\\n` sequences in private keys are converted to newlines before authentication.

### 14.5 Professional notification email

Notification emails use an email-client-safe, table-based layout with a branded dark header, color-coded severity badge, aligned metric cards, root-cause summary, failing-bin table, MES telemetry section, design assessment panel, shipment-risk card, and plain-text fallback. Dynamic content is HTML-escaped before insertion.

### 14.6 Version 2 example requests

```text
Preview notification emails for LOT-91456 to the Design Lead from the Stakeholders sheet. Do not send.
```

```text
Send the LOT-91456 incident notification to all stakeholders from the Stakeholders sheet. Do not filter by severity.
```

Structured inputs:

```json
{ "lotId": "LOT-91456", "recipientRoles": ["Design Lead"], "dryRun": true }
```

```json
{ "lotId": "LOT-91456", "allStakeholders": true, "dryRun": false }
```
